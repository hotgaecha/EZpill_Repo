...
def lambda_handler(event, context):
    ...
    try:
        ...
        # 사용자의 결과 가져오기
        query = f"SELECT recommendation FROM SurveyResponses WHERE firebase_uid = '{firebase_uid}'"
        ...
        for row in recommendation:
            for element in row:
                ...
                modified_recommendation.extend(element.split(','))

        # 제품 테이블 이름 생성
        tables = [f"product_{item.strip()}" for item in modified_recommendation]
        ...
        for table in tables:
            ...
            cursor.execute(query)
            top_products_info[table] = [{'Product_Title': item[0], 'Product_Per_Price': item[1]} for item in
                                        cursor.fetchall()]
        ...
        cursor.execute(query)
        investment_recommendation = cursor.fetchone()
        if not investment_recommendation or not str(investment_recommendation[0]).isdigit():
            return {
                'statusCode': 404,
                'body': json.dumps({'error': 'Investment information not found or invalid'})
            }

        investment = int(investment_recommendation[0])

        # 최소 비용 조합 계산
        min_cost_combination, total_cost_min = calculate_min_cost_combination(top_products)
        # 추가 비용 포함 조합 계산
        cost_up_combination, total_cost_up = calculate_cost_up_combination(top_products, investment,
        ...
        return {
            'statusCode': 200,
            'body': json.dumps(final_recommendation)
        }
...
def calculate_min_cost_combination(top_products):
    ...
    for category, products in top_products.items():
        if not products:  # 제품 목록이 비어 있는 경우 해당 카테고리를 무시
            print(f"No products available in category {category}")
            continue
...
def calculate_cost_up_combination(top_products, investment, min_cost_combination):
    ...
    total_cost = sum(
        int(product['Product_Per_Price'].replace(',', '')) if isinstance(product['Product_Per_Price'], str) else
        product['Product_Per_Price'] for product in cost_up_combination.values())
    ...
    for category, products in top_products.items():
        ...
        for product in products:
            product_price = int(product['Product_Per_Price'].replace(',', '')) if isinstance(
                product['Product_Per_Price'], str) else product['Product_Per_Price']  # 쉼표 제거 및 정수 변환
            current_price = int(cost_up_combination[category]['Product_Per_Price'].replace(',', '')) if isinstance(
                cost_up_combination[category]['Product_Per_Price'], str) else cost_up_combination[category][
                'Product_Per_Price']
