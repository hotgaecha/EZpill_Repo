...
class Recommendation:
    def __init__(self, firebase_uid, name, birth_date, height, weight, pregnancy_week, supplements, medications, allergies, chronic_diseases, health_concerns, investment, created_at, recommendation):
    ...
    def make_recommend_combination(self):
        ...
        # 1. 임신 주차에 따른 영양제
        if 1 <= self.pregnancy_week <= 12:
            pill_list.extend(["비타민 D", "유산균", "엽산"])
        elif 16 <= self.pregnancy_week:
            pill_list.extend(["비타민 D", "유산균", "철분"])
        elif 20 <= self.pregnancy_week:
            pill_list.extend(["비타민 D", "유산균", "철분", "칼슘"])
        elif 28 <= self.pregnancy_week:
            pill_list.extend(["비타민 D", "유산균", "철분", "칼슘", "오메가-3"])
    ...
    # 중복 제거
        ...
        return pill_list
    def save_recommendation_to_database(self, recommendation):
        ...
        update_query = "UPDATE SurveyResponses SET recommendation = %s WHERE firebase_uid = %s"
        cursor.execute(update_query, (', '.join(recommendation), self.firebase_uid))
...
def lambda_handler(event, context):
    ...
    if survey_data:
        ...
        recommendation = survey_instance.make_recommend_combination()
        survey_instance.save_recommendation_to_database(recommendation)

        # Return user name and recommendation
        return {
            'statusCode': 200,
            'body': json.dumps({'name': survey_instance.name, 'recommendation': ', '.join(recommendation)})
        }
    else:
