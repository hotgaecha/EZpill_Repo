...
try:
    ...
    if connection.is_connected():
        ...
        # 예시: 영양제 정보를 가져오는 쿼리
        query = "SELECT Product_Title, Product_Per_Price FROM product_비타민d"
        cursor.execute(query)

        # 결과 가져오기
        supplement_data = cursor.fetchall()

        # supplement_prices 업데이트
        supplement_prices = {Product_Title: Product_Per_Price for Product_Title, Product_Per_Price in supplement_data}
...
class Recommendation:
    ...
    def __init__(self, weeks, age, bmi, medication, current_supplements, allergies, chronic_conditions,
...
# weeks, age, bmi, medication, current_supplements, allergies, chronic_conditions,
# digestive_issues, muscle_twitching, eye_fatigue, skin_condition, feel_down, pill_preference, budget):
...
chronic_conditions = input("만성질환 또는 지병이 있나요? (없으면 엔터): ")
...
recommendation = Recommendation(weeks, age, bmi, medication, current_supplements, allergies, chronic_conditions,
