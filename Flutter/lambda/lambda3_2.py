...
def lambda_handler(event, context):
    ...
    try:
        ...
        for table in tables:
            ...
            cursor.execute(query)
            top_products[table] = cursor.fetchall()
        # 사용자의 예산 가져오기
        ...
        # 최소 비용 조합 계산
        min_cost_combination, total_cost_min = calculate_min_cost_combination(
            top_products)
        # 추가 비용 포함 조합 계산
        cost_up_combination, total_cost_up = calculate_cost_up_combination(
            top_products, investment, min_cost_combination)
...
def calculate_min_cost_combination(top_products):
    ...
    for category, products in top_products.items():
        min_product = min(products, key=lambda x: x[1])  # 가장 저렴한 제품 선택
        min_cost_combination[category] = {
            'Product_Title': min_product[0],
            'Product_Per_Price': min_product[1]
        }

        # min_product[1]이 문자열인 경우에만 쉼표 제거
        if isinstance(min_product[1], str):
            price_without_comma = min_product[1].replace(',', '')
        else:
            price_without_comma = min_product[1]
...
def calculate_cost_up_combination(top_products, investment, min_cost_combination):
    ...
    total_cost = sum(int(product['Product_Per_Price'].replace(',', '')) if isinstance(
        product['Product_Per_Price'], str) else product['Product_Per_Price'] for product in cost_up_combination.values())
    ...
    for category, products in top_products.items():
        ...
        for product in products:
            product_price = int(product[1].replace(',', '')) if isinstance(
                product[1], str) else product[1]  # 쉼표 제거 및 정수 변환
            current_price = int(cost_up_combination[category]['Product_Per_Price'].replace(',', '')) if isinstance(
                cost_up_combination[category]['Product_Per_Price'], str) else cost_up_combination[category]['Product_Per_Price']
